...
└── src/
    ...
    ├── components/
    │   ├── Icons.jsx             # 대시보드용 SVG 아이콘 24개
    │   ├── InstagramIcons.jsx    # Instagram 미리보기용 SVG 아이콘 19개
    │   ├── PostCard.jsx          # 대시보드 포스트 카드 (상태 배지 포함)
    │   ├── WeekNav.jsx           # 동적 주차 네비게이션 (새 주차 추가 버튼)
    │   ├── WeekWizard.jsx        # 새 주차 생성 3단계 위저드
    │   ├── PostDetail.jsx        # 포스트 상세 모달 (발행 체크리스트 포함)
    │   ├── PostEditor.jsx        # 포스트 생성/수정 에디터 (가이드 패널)
    │   ├── ProfileHeader.jsx     # Instagram 프로필 헤더
    │   ├── GridView.jsx          # Instagram 3열 그리드 (필터 지원)
    │   └── FeedDetailView.jsx    # Instagram 피드 상세
    ├── data/
    │   └── posts.js              # strategyData + feedPosts (fallback 하드코딩)
    └── lib/
        ├── supabase.js           # Supabase 클라이언트 + DB 함수 모음
        └── templates.js          # 캡션 템플릿 엔진 (API 불필요)
```
...
- 동적 주차 관리: 12주 기본 + "새 주차 추가" 위저드로 무제한 확장
- 포스트 카드에 상태 배지 (초안/준비완료/발행됨)
- 발행 현황 진행률 바 (N/총N 발행 완료)
- 포스트 CRUD (생성, 수정, 삭제), 이미지 업로드 (Supabase Storage)
- 포스트 에디터에 콘텐츠 가이드 패널 (요일별 가이드 + 캐러셀 슬라이드 구성 가이드)
...
- 발행 상태별 필터링 (전체/발행됨/미발행)
- Draft 포스트 반투명 표시 + DRAFT 배지
- DB 데이터 우선, 실패 시 하드코딩 feedPosts 폴백

### card_news_maker.html
카드뉴스 슬라이드 제작 도구 (Standalone, html2canvas 포함).
- 캘린더 선택 / 직접 입력 / **템플릿 프리셋** 3가지 제작 모드
- 템플릿 프리셋 4종: 가이드형, 프로그램 소개형, 전문가 소개형, Q&A 팩트체크형
- 슬라이드별 가이드 텍스트 (플레이스홀더)로 구성 방향 안내
- 브랜드 모드: IM에스테틱 브랜드 컬러 팔레트 고정
- Unsplash 이미지 자동 매칭, html2canvas 개별/전체 다운로드
...
| publish_checklist | jsonb | 발행 체크리스트 (caption_copied, image_downloaded, uploaded_to_instagram, hashtags_added, published_at) |
...
이 시스템은 현재 **IM에스테틱 전용**이지만, 처음부터 **멀티테넌트 구조**로 설계되어 다른 클리닉으로의 확장이 용이합니다.
...
| 기능 | 설명 |
|------|------|
| 동적 주차 확장 | 12주 이후 무제한 주차 추가 (WeekWizard 3단계 위저드) |
| 캡션 템플릿 엔진 | API 없이 프로그램/요일별 캡션 자동 생성 (`templates.js`) |
| 콘텐츠 가이드 패널 | PostEditor에서 요일/유형별 제작 가이드 + 참고 포스트 표시 |
| 발행 체크리스트 | 4단계 체크박스 (캡션 복사→이미지 다운로드→업로드→해시태그) |
| 발행 현황 트래킹 | 대시보드 진행률 바 + PostCard 상태 배지 |
| 카드뉴스 프리셋 | 4종 템플릿 (가이드형/프로그램형/전문가형/Q&A형) + 슬라이드 가이드 |
| 피드 프리뷰 필터 | 발행 상태별 필터 (전체/발행됨/미발행) |
