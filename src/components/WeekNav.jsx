...
import { Calendar, ChevronRight, Plus } from './Icons.jsx';

export const WeekNav = ({ strategyData, activeWeek, onSelect, onAddWeek, dbConnected }) => (
    <nav className="lg:col-span-3 space-y-3">
        ...
        <div className="max-h-[calc(100vh-200px)] overflow-y-auto space-y-3 no-scrollbar pr-1">
                ...
                <button key={item.week} onClick={() => onSelect(item.week)}
                    ...
                    <div className="flex items-center gap-3">
                        <span className={`text-[10px] font-black w-7 h-7 rounded-full flex items-center justify-center border ${activeWeek === item.week ? 'border-orange-400 text-white' : 'bg-gray-100 border-gray-200'}`}
                            style={activeWeek === item.week ? {background:'#E8703A'} : {}}>
                            {String(item.week).padStart(2,'0')}
                        </span>
                        <div className="text-left">
                            <div className="text-[10px] opacity-60 font-bold uppercase tracking-tight italic">Week</div>
                            <div className="font-bold text-sm leading-tight truncate max-w-[140px]">{item.theme || `주차 로드맵 ${String(item.week).padStart(2,'0')}`}</div>
                        </div>
                    </div>
                    <ChevronRight size={16} className={activeWeek === item.week ? '' : 'text-gray-300'} style={activeWeek === item.week ? {color:'#E8703A'} : {}} />
                </button>
            ))}
        </div>
        {dbConnected && onAddWeek && (
            <button onClick={onAddWeek}
                className="w-full flex items-center justify-center gap-2 p-4 rounded-2xl border-2 border-dashed border-gray-200 text-gray-400 hover:border-orange-300 hover:text-orange-500 transition-all duration-300 group">
                <Plus size={16} />
                <span className="text-sm font-bold">새 주차 추가</span>
            </button>
        )}
    </nav>
);
