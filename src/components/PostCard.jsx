...
const STATUS_CONFIG = {
    draft: { label: '초안', bg: 'bg-gray-100', text: 'text-gray-500', border: 'border-gray-200' },
    ready: { label: '준비완료', bg: 'bg-blue-50', text: 'text-blue-600', border: 'border-blue-100' },
    published: { label: '발행됨', bg: 'bg-green-50', text: 'text-green-600', border: 'border-green-100' },
};
...
export const PostCard = ({ post, idx, activeWeek, onSelect }) => {
    ...
    return (
        <button onClick={() => onSelect(post)}
            ...
            {post.status && (
                <div className={`absolute top-3 right-3 z-10 px-2 py-0.5 rounded-full text-[10px] font-black border ${status.bg} ${status.text} ${status.border}`}>
                    {status.label}
                </div>
            )}
            {post.image && (
                <div className={`w-full h-36 bg-gray-100 overflow-hidden ${post.status === 'draft' ? 'opacity-60' : ''}`}>
